...
export async function fetchEvents(params?: {
  ...
  const res = await fetch(`${BASE}/api/events?${q}`, { cache: "no-store" });
  return res.json();
}

export async function fetchTimeline(hours = 24) {
  const res = await fetch(`${BASE}/api/timeline?hours=${hours}`, { cache: "no-store" });
  return res.json();
}

export async function fetchAttackTypes() {
  const res = await fetch(`${BASE}/api/attack-types`, { cache: "no-store" });
  return res.json();
}
